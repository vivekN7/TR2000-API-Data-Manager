-- ===============================================================================
-- TR2000_UTIL Package - Fixed Version
-- Date: 2025-08-27
-- Purpose: DBA's centralized API utility package with proper schema references
-- ===============================================================================
...
CREATE OR REPLACE PACKAGE BODY tr2000_util AS
  ...
  BEGIN
    -- Use DBMS_CRYPTO for hashing (requires grant)
    RETURN RAWTOHEX(DBMS_CRYPTO.HASH(
      UTL_I18N.STRING_TO_RAW(p_json, 'AL32UTF8'),
      DBMS_CRYPTO.HASH_SH256
    ));
  EXCEPTION
    WHEN OTHERS THEN
      -- Fallback to simple hash if DBMS_CRYPTO not available
      RETURN TO_CHAR(ORA_HASH(SUBSTR(p_json, 1, 4000)));
  END hash_json;
  ...
  BEGIN
    ...
    IF p_payload IS NOT NULL THEN
      ...
      VALUES (
        p_endpoint,
        SUBSTR(TO_CHAR(ORA_HASH(NVL(p_query_params,'-')||'|'||p_endpoint)),1,64),
        p_payload,
        p_batch_id
      );
    END IF;
  ...
  BEGIN
    ...
    -- Note: For now, using basic authentication without credentials
    -- The credential will be set up in APEX workspace later
    l_resp := apex_web_service.make_rest_request(
      p_url         => l_url,
      p_http_method => 'GET'
      -- p_credential_static_id => p_cred_id  -- Uncomment when credential is created
    );
...
PROMPT ===============================================================================
PROMPT 
PROMPT Next steps for DBA:
PROMPT 1. Create APEX credential 'TR2000_CRED' for API authentication
PROMPT 2. Grant execute on tr2000_util to TR2000_STAGING
PROMPT 3. Grant necessary network ACLs to the owner of tr2000_util
PROMPT ===============================================================================