...
-- CONTROL_ENDPOINT_STATE: Tracks endpoint processing state
CREATE TABLE CONTROL_ENDPOINT_STATE (
    ...
    CONSTRAINT UK_ENDPOINT_STATE UNIQUE (endpoint_key, plant_id)
);

COMMENT ON TABLE CONTROL_ENDPOINT_STATE IS 'Tracks the current state and last successful run for each endpoint';

CREATE INDEX IDX_STATE_ENDPOINT ON CONTROL_ENDPOINT_STATE(endpoint_key);
CREATE INDEX IDX_STATE_LOCKED ON CONTROL_ENDPOINT_STATE(is_locked);
