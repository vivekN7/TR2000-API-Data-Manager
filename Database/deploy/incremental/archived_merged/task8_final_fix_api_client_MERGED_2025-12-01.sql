-- ===============================================================================
-- Final fix for PKG_API_CLIENT_PCS_DETAILS_V2
-- Date: 2025-08-29
-- Purpose: Use actual issue_revision from SELECTED_ISSUES, not dummy values
-- ===============================================================================

CREATE OR REPLACE PACKAGE BODY pkg_api_client_pcs_details_v2 AS

    -- Get base URL (unchanged)
    ...
    -- Fetch ALL PCS for a plant (unchanged)
    ...
    -- Refresh PCS list for a plant (unchanged)
    ...
    -- Fetch PCS detail JSON (unchanged)
    ...
    -- FIXED: Refresh PCS details using actual issue_revision
    PROCEDURE refresh_pcs_details(
        ...
        l_issue_rev VARCHAR2(50);
    BEGIN
        -- Get the actual issue_revision for this plant
        BEGIN
            SELECT issue_revision INTO l_issue_rev
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                p_status := 'ERROR';
                p_message := 'No active issue found for plant ' || p_plant_id;
                RETURN;
        END;
        ...
        INSERT INTO RAW_JSON (
            ...
            l_endpoint_key,
            p_plant_id,
            l_issue_rev,  -- Using actual issue_revision
            l_json,
            l_hash,
            l_correlation_id
        ) RETURNING raw_json_id INTO l_raw_json_id;
        ...
        pkg_parse_pcs_details.parse_pcs_detail_json(
            ...
            p_raw_json_id => l_raw_json_id,
            p_plant_id => p_plant_id,
            p_issue_rev => l_issue_rev,  -- Using actual issue_revision
    ...
    -- Refresh ALL detail types for a PCS revision (unchanged except calls refresh_pcs_details)
