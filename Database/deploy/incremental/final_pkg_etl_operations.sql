-- ===============================================================================
-- Final Working Version of PKG_ETL_OPERATIONS
-- Date: 2025-08-29
-- Purpose: Complete ETL orchestration with separated procedures
-- ===============================================================================
...
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(*) INTO v_count FROM user_sequences WHERE sequence_name = 'ETL_RUN_LOG_SEQ';
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE SEQUENCE etl_run_log_seq START WITH 1 INCREMENT BY 1';
    END IF;
END;
/

-- Create the complete package body
CREATE OR REPLACE PACKAGE BODY pkg_etl_operations AS
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('Plants loaded: ' || p_status);
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('Loading issues for ' || v_plant_count || ' selected plants...');
        ...
        FOR plant IN (SELECT sp.plant_id, p.short_description
            ...
            IF v_plant_status = 'SUCCESS' THEN
                DBMS_OUTPUT.PUT_LINE('  Plant ' || plant.plant_id || ' (' || plant.short_description || '): SUCCESS');
            ELSE
    ...
    BEGIN
        ...
        -- Step 3b: Load PCS list for each unique plant
        DBMS_OUTPUT.PUT_LINE('Loading PCS lists...');
        FOR plant IN (SELECT DISTINCT si.plant_id
        ...
        FOR pcs IN (SELECT DISTINCT pr.plant_id, pr.pcs_name, pr.official_revision
            ...
            -- Call refresh_all_pcs_details which loads all 6 detail types
            pkg_api_client_pcs_details.refresh_all_pcs_details(
                p_plant_id => pcs.plant_id,
                p_pcs_name => pcs.pcs_name,
                p_pcs_revision => pcs.official_revision,
                p_status => v_pcs_status,
                p_message => v_pcs_msg
            );
    ...
    BEGIN
        ...
        -- Clean detail tables first (leaf nodes)
        ...
        -- Clean staging tables (no count needed)
    ...
    BEGIN
        ...
        IF v_plant_exists = 0 THEN
            p_status := 'ERROR';
            p_message := 'Plant ' || p_plant_id || ' not found. Load plants first.';
            RETURN;
        END IF;
    ...
    BEGIN
        ...
        IF v_issue_exists = 0 THEN
            p_status := 'ERROR';
            p_message := 'Issue ' || p_plant_id || '/' || p_issue_revision || ' not found. Load issues first.';
            RETURN;
        END IF;
    ...
    BEGIN
        ...
        FOR plant IN (SELECT sp.plant_id, p.short_description
                      FROM SELECTED_PLANTS sp
                      LEFT JOIN PLANTS p ON p.plant_id = sp.plant_id
                      WHERE sp.is_active = 'Y'
                      ORDER BY sp.plant_id) LOOP
            DBMS_OUTPUT.PUT_LINE('  ' || plant.plant_id || 
                                 CASE WHEN plant.short_description IS NOT NULL 
                                      THEN ' - ' || plant.short_description 
                                      ELSE '' END);
        END LOOP;
        ...
        FOR issue IN (SELECT si.plant_id, si.issue_revision, i.rev_date
                      FROM SELECTED_ISSUES si
                      LEFT JOIN ISSUES i ON i.plant_id = si.plant_id 
            ...
            DBMS_OUTPUT.PUT_LINE('  ' || issue.plant_id || '/' || issue.issue_revision || 
                                 CASE WHEN issue.rev_date IS NOT NULL
                                      THEN ' (Date: ' || TO_CHAR(issue.rev_date, 'YYYY-MM-DD') || ')'
                                      ELSE '' END);
        END LOOP;
    ...
    -- =========================================================================
    -- Legacy procedures (for backwards compatibility)
    -- =========================================================================
...
-- Verify compilation
SHOW ERRORS PACKAGE BODY pkg_etl_operations
