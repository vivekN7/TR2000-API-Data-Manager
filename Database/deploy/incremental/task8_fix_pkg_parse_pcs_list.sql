-- ===============================================================================
-- Update PKG_PARSE_PCS_DETAILS to include PCS_LIST parsing
-- Date: 2025-08-29
-- Purpose: Add parsing for plant-level PCS list (endpoint 3.1)
-- ===============================================================================

CREATE OR REPLACE PACKAGE pkg_parse_pcs_details AS
    -- NEW: Parse PCS list from plant endpoint (3.1)
    PROCEDURE parse_plant_pcs_list(
        p_raw_json_id    IN NUMBER,
        p_plant_id       IN VARCHAR2
    );
    
    -- Existing procedures for PCS details
...
CREATE OR REPLACE PACKAGE BODY pkg_parse_pcs_details AS
    ...
    -- =========================================================================
    -- NEW: Parse plant-level PCS list (endpoint 3.1)
    -- =========================================================================
    PROCEDURE parse_plant_pcs_list(
        p_raw_json_id    IN NUMBER,
        p_plant_id       IN VARCHAR2
    ...
    BEGIN
        ...
        -- The API returns an array of PCS objects
        ...
        DBMS_OUTPUT.PUT_LINE('Parsed ' || v_record_count || ' PCS list records for plant ' || p_plant_id);
        
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20360, 
                'Error parsing plant PCS list JSON: ' || SQLERRM);
    END parse_plant_pcs_list;

    -- Include all existing procedures here (unchanged)
    -- parse_header_properties, parse_temp_pressures, parse_pipe_sizes, 
    -- parse_pipe_elements, parse_valve_elements, parse_embedded_notes, parse_pcs_detail_json
    -- (copying from original package body...)
