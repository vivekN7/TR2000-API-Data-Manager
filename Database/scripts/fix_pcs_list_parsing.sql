-- Fix for PCS_LIST parsing - corrects JSON path from $.getPlantPcsList to $.getPCS
-- Date: 2025-09-05
-- Issue: PCS_LIST not loading, STG_PCS_LIST has only null values
-- Root Cause: Wrong JSON path - actual API returns $.getPCS[*] not $.getPlantPcsList[*]

-- First, let's verify the current state
PROMPT ====================================
PROMPT Checking current PCS_LIST data before fix
PROMPT ====================================
SELECT COUNT(*) as stg_count FROM STG_PCS_LIST;
SELECT COUNT(*) as stg_with_pcs FROM STG_PCS_LIST WHERE "PCS" IS NOT NULL;
SELECT COUNT(*) as core_count FROM PCS_LIST;

-- Now update ONLY the parse_and_load_pcs_list procedure in PKG_ETL_PROCESSOR
...
PROMPT ====================================
PROMPT Testing parse_and_load_pcs_list fix
PROMPT ====================================
SET SERVEROUTPUT ON

DECLARE
    v_raw_json_id NUMBER := 4119; -- The PCS_LIST entry in RAW_JSON
BEGIN
    DBMS_OUTPUT.PUT_LINE('Testing parse_and_load_pcs_list with raw_json_id=' || v_raw_json_id);
    PKG_ETL_PROCESSOR.parse_and_load_pcs_list(v_raw_json_id, '34');
...
PROMPT ====================================
PROMPT Verifying results
PROMPT ====================================
SELECT COUNT(*) as stg_count FROM STG_PCS_LIST WHERE "PCS" IS NOT NULL;
SELECT COUNT(*) as core_count FROM PCS_LIST;

-- Show sample data
PROMPT ====================================
PROMPT Sample PCS_LIST data (first 5 rows)
PROMPT ====================================
SELECT pcs_name, revision, status, rating_class
FROM PCS_LIST 
WHERE ROWNUM <= 5;
