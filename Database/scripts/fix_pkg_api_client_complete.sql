-- Complete PKG_API_CLIENT using API_SERVICE.API_GATEWAY proxy
CREATE OR REPLACE PACKAGE BODY PKG_API_CLIENT AS
    ...
    -- Fetch PCS detail (uses fetch_pcs_detail_data internally)
    ...
    BEGIN
        -- Just call the existing fetch_pcs_detail_data function
        RETURN fetch_pcs_detail_data(
            p_plant_id => p_plant_id,
    ...
    -- Fetch PCS detail data (internal function - used by fetch_pcs_detail)
    ...
    BEGIN
        ...
        -- Store in RAW_JSON (fixed column list)
        INSERT INTO RAW_JSON (
            raw_json_id,
            endpoint_key,
            endpoint_template,
        ...
        -- Log error if HTTP status is not success (fixed column reference)
        IF v_http_status IS NULL OR v_http_status NOT BETWEEN 200 AND 299 THEN
            INSERT INTO ETL_ERROR_LOG (
                error_id,
                endpoint_key,
