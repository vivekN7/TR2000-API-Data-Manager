-- Fix PKG_MAIN_ETL_CONTROL with correct parameter names
-- The issue: fetch_pcs_detail expects p_revision (not p_pcs_revision)
-- Also completes the rename from run_full_etl to run_main_etl
...
PROMPT ====================================
PROMPT Testing run_main_etl
PROMPT ====================================
SET SERVEROUTPUT ON

-- Reset first for clean test
EXEC PKG_ETL_TEST_UTILS.reset_for_testing;

-- Now test run_main_etl
EXEC PKG_MAIN_ETL_CONTROL.run_main_etl;

-- Verify results
PROMPT ====================================
PROMPT Verifying ETL results
PROMPT ====================================
