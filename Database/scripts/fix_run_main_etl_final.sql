-- Fix the fetch_pcs_detail call in process_pcs_details
-- Also keep the rename from run_full_etl to run_main_etl
    ...
    BEGIN
        ...
        FOR rec IN (
            ...
            FOR detail_type IN (
                ...
                -- Fixed: fetch_pcs_detail expects p_pcs_revision not p_revision
                v_raw_json_id := PKG_API_CLIENT.fetch_pcs_detail(
                    p_plant_id => rec.plant_id,
                    p_pcs_name => rec.pcs_name,
                    p_pcs_revision => rec.revision,  -- Changed from p_revision
