-- Rename run_full_etl to run_main_etl in PKG_MAIN_ETL_CONTROL
-- Date: 2025-09-05
-- Safe rename - run_full_etl is not referenced by any other packages

-- Step 1: Get current package spec
...
-- Step 2: Update package body - just rename the procedure
CREATE OR REPLACE PACKAGE BODY PKG_MAIN_ETL_CONTROL AS

    -- Get current package body and rename procedure
    PROCEDURE run_main_etl IS  -- Changed from run_full_etl
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            RAISE;
    END run_main_etl;  -- Changed from run_full_etl
    ...
    BEGIN
        ...
        -- Clear existing data for this plant/issue
        ...
        FOR ref_type IN (
            ...
            -- Parse and load data based on type
    ...
    BEGIN
        ...
        -- Process unique PCS combinations
        FOR rec IN (
            SELECT DISTINCT 
                l.plant_id,
                l.pcs_name,
            ...
            FOR detail_type IN (
                ...
                -- Process detail through PKG_PCS_DETAIL_PROCESSOR
...
-- Test the renamed procedure
PROMPT Testing renamed procedure...
SET SERVEROUTPUT ON
...
BEGIN
    ...
    -- Test the new name
    DBMS_OUTPUT.PUT_LINE('Testing PKG_MAIN_ETL_CONTROL.run_main_etl...');
    PKG_MAIN_ETL_CONTROL.run_main_etl;
    
    -- Check results
    FOR rec IN (
        SELECT 'PCS_REFERENCES' as tbl, COUNT(*) as cnt FROM PCS_REFERENCES
        UNION ALL
        SELECT 'PCS_LIST', COUNT(*) FROM PCS_LIST
    ) LOOP
        DBMS_OUTPUT.PUT_LINE(rec.tbl || ': ' || rec.cnt || ' records');
    END LOOP;
END;
/

PROMPT Rename complete! Now use: EXEC PKG_MAIN_ETL_CONTROL.run_main_etl;