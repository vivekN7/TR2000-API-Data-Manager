-- Simplified PKG_API_CLIENT that calls APEX_WEB_SERVICE directly
-- No need for PKG_HTTP_UTIL intermediate layer
...
CREATE OR REPLACE PACKAGE BODY PKG_API_CLIENT AS
    ...
    BEGIN
        ...
        -- Make HTTPS call using APEX_WEB_SERVICE directly
        BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                v_http_status := NVL(apex_web_service.g_status_code, -1);
    ...
    BEGIN
        ...
        -- Get template for logging
        v_template := build_endpoint_url('PCS_LIST', '{plant_id}');
        ...
        -- Store in RAW_JSON with fingerprint
        INSERT INTO RAW_JSON (
            raw_json_id,
            endpoint_key,
            endpoint_template,
    ...
    BEGIN
        ...
        -- Build actual path
        v_path := build_endpoint_url(p_endpoint_key => 'VDS_LIST');
        
        -- Get template for logging
        v_template := v_path;  -- VDS_LIST has no parameters
