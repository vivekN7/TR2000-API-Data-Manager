...
-- RAW_JSON table for API response audit trail
...
-- RAW_JSON insert procedure
CREATE OR REPLACE PROCEDURE SP_INSERT_RAW_JSON(
    p_etl_run_id NUMBER,
    p_endpoint_name VARCHAR2,
    p_plant_id VARCHAR2,
    p_json_data CLOB
) AS
    v_blob BLOB;
    ...
    v_warning INTEGER;
BEGIN
    -- Convert CLOB to BLOB for compressed storage
    DBMS_LOB.CREATETEMPORARY(v_blob, TRUE);
    
    -- Use correct CONVERTTOBLOB signature for Oracle 21c
    DBMS_LOB.CONVERTTOBLOB(
        dest_lob => v_blob,
        src_clob => p_json_data,
    ...
    INSERT INTO RAW_JSON (ETL_RUN_ID, ENDPOINT_NAME, PLANT_ID, JSON_DATA)
    VALUES (p_etl_run_id, p_endpoint_name, p_plant_id, v_blob);
